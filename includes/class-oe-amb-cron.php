...
class OE_Amb_Cron {
	...
	public static function schedule_events(): void {
		if ( ! wp_next_scheduled( 'oe_amb_monthly_reports' ) ) {
			...
			$next = mktime( 8, 0, 0, (int) gmdate( 'n' ) + 1, 1 );
	...
	public function auto_approve_old_commissions(): void {
		...
		$table  = OE_Amb_DB::com_table();

		$wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
				"UPDATE `{$table}` SET status = 'approved' WHERE status = 'pending' AND order_date < %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$cutoff
			)
		);
	}
